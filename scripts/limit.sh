...
#   cost_per_day    estimated USD cost per UTC day
...
# Usage:
#   hr limit list
#   hr limit set <key> [--rpm N] [--req-day N] [--tokens-day N] [--cost-day USD]
#   hr limit clear <key>
#
set -uo pipefail
...
if not pk:
    ...
    for k, v in pk.items():
        ...
        ct  = v.get("cost_per_day", 0) or 0
        cts = f"${ct:g}" if ct else "∞"
        print(f"  …{tail}:  rpm={rpm}  req/day={rq}  tokens/day={tk}  cost/day={cts}")
...
print(f"  PROXY_LIMIT_RPM={os.environ.get('PROXY_LIMIT_RPM','0')}  "
      ...
      f"PROXY_LIMIT_COST_DAY={os.environ.get('PROXY_LIMIT_COST_DAY','0')}")
PY
    ;;

  set)
    key="${1:-}"; shift 2>/dev/null || true
    [ -n "$key" ] || { err "usage: hr limit set <key> [--rpm N] [--req-day N] [--tokens-day N] [--cost-day USD]"; exit 1; }
    rpm=""; reqday=""; tokday=""; costday=""
    while [ $# -gt 0 ]; do
      case "$1" in
        --rpm)        rpm="${2:-}"; shift 2 ;;
        --req-day)    reqday="${2:-}"; shift 2 ;;
        --tokens-day) tokday="${2:-}"; shift 2 ;;
        --cost-day)   costday="${2:-}"; shift 2 ;;
        *) err "unknown option: $1"; exit 1 ;;
      esac
    done
    AUTH_FILE="$AUTH_FILE" KEY="$key" RPM="$rpm" REQDAY="$reqday" TOKDAY="$tokday" COSTDAY="$costday" "$PYTHON" - <<'PY'
import json, os
f = os.environ["AUTH_FILE"]
try:
    doc = json.load(open(f))
...
for env, field in (("RPM", "rpm"), ("REQDAY", "req_per_day"), ("TOKDAY", "tokens_per_day")):
    ...
    if val != "":
        spec[field] = int(val)
cd = os.environ.get("COSTDAY", "")
if cd != "":
    spec["cost_per_day"] = float(cd)
