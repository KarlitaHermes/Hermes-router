...
description: "Hands-on: go from a chatbot to a memory-backed, tool-using agent — copy-paste code."
---
...
> New to the words here (LLM, token, tool…)? Skim **[concepts.md](/concepts/)** first.
...
1. hermes-router is installed and running (see **[getting-started.md](/getting-started/)**).
...
while True:
    ...
    reply = client.chat.completions.create(model="hermes-router", messages=messages)
...
while True:
    reply = client.chat.completions.create(
        model="hermes-router", messages=messages, tools=tools,
    )
...
- **Keep the context tight.** Send what the model needs, not everything. Long histories cost
  tokens and can dilute focus (hermes-router caches and the host app can summarize).
...
- **More usage patterns:** **[usage.md](/usage/)** (Anthropic SDK, embeddings, streaming).
